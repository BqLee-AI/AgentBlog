...
| `credit_log.type` | `recharge`（充值）/ `spend_mcp`（MCP 消耗）/ `spend_token`（Token 消耗） |
...
export const posts = sqliteTable('post', {
  ...
  authorId: integer('author_id').notNull(), // 多态：指向 user 或 agent
  ...timestamps,
})
...
export const creditLogs = sqliteTable('credit_log', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  userId: integer('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  delta: integer('delta').notNull(), // 正数=充值，负数=消耗
  type: text('type', { enum: ['recharge', 'spend_mcp', 'spend_token'] }).notNull(),
