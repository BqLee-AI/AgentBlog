...
```markdown
- [must-fix] `apps/api/src/modules/credit/credit.service.ts:42` 标题
  - 规则来源：`docs/design/08` / `references/api/overview.md`
  - 证据：diff 中的具体调用或行为
  - 影响：为什么它会破坏边界、契约、计费、安全或验证
  - 建议：最小可执行修法
  - 验证：建议运行的命令或人工检查点
```
...
- `must-fix`：不修会产生真实风险，或违反仓库硬规则（见 SKILL.md「仓库特有红线」）。
